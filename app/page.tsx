...
function money(value: number) {
  return new Intl.NumberFormat("en-IE", {
    ...
    currency: "EUR",
    notation:
      Math.abs(value) >= 1000 ? "compact" : "standard",
...
function signalDescription(
  ...
  switch (signal.code) {
    case "SCOPE_DRIFT":
      return `${target}: ${n(
        d.hours_logged
      ).toFixed(1)}h logged against ${n(
        d.planned_hours
      ).toFixed(1)}h planned plus ${n(
        d.approved_co_hours
      ).toFixed(1)}h approved scope.`;

    case "OVER_BUDGET":
      return `${target}: budget utilization is ${n(
        d.burn_pct
      ).toFixed(1)}%.`;
    ...
    case "UNRECONCILED":
      return `${target}: invoice total ${money(
        n(d.invoice_amount)
      )} does not match line total ${money(
        n(d.lines_sum)
      )}.`;
  }
}
...
export default async function Home() {
  ...
  const readFailures: ReadFailure[] = [
    ...
    .map((item) => ({
      source: item.source,
      code:
        typeof item.error.code ===
        "string"
          ? item.error.code
          : null,
      message:
        item.error.message ||
        "Unknown database read error",
    }));

  if (readFailures.length > 0) {
    console.error(
      "[Signet] Command Center live data read failure",
      readFailures
    );
  }
  ...
  function engagementIdForSignal(
    signal: SignalRow
  ) {
    if (
      signal.stream_type ===
    ...
    if (
      ...
      return invoiceToEngagement.get(
        signal.stream_id
      );
    }
  ...
  const signalsByEngagement =
    new Map<string, SignalRow[]>();

  for (const signal of signals) {
    const engagementId =
      engagementIdForSignal(signal);
    ...
    const existing =
      signalsByEngagement.get(
        engagementId
      ) ?? [];

    existing.push(signal);

    signalsByEngagement.set(
      engagementId,
      existing
    );
  }
  ...
  const totalUnbilled =
    engagements.reduce(
      (sum, row) =>
        sum +
        n(row.unbilled_amount),
      0
    );

  const unbilledSignals =
    signals.filter(
      (signal) =>
        signal.code === "UNBILLED"
    );
      ...
      <div className="content">
        <section className="hero">
          ...
          <p>
            ...
            derived from authenticated
            deterministic projections
            and operational signals.
            The narration layer remains
        ...
        {hasDataError ? (
          ...
          >
            ...
            >
              ...
              >
                Signet is refusing to
                substitute hard-coded
                demo values.
              </p>
          ...
          <div className="panel">
            ...
            {healthRows.length === 0 ? (
              ...
              >
                ...
                >
                  Append operational
                  events and this view
                  will update from the
                  projection layer.
                </p>
              </div>
            ) : (
        ...
        <section className="ai-panel">
          <div className="ai-copy">
            ...
            <small>
              Current local narration
              is generated from
              deterministic signal
              fields only. A future
              LLM adapter may explain
              this evidence, but it
              will not receive
              authority to mutate
              operational state.
            </small>
